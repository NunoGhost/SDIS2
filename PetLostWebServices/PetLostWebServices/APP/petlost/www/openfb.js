var openFB = (function () {
    ...
    function login(callback, options) {
        ...
        var url=loginURL + '?client_id=' + fbAppId + '&redirect_uri=' + redirectURL +'&response_type=token&scope=' + scope;
        loginWindow = window.open(url, '_blank', 'location=no,clearcache=yes');
    ...
    function logout(callback) {
        ...
        if (token) {
            logoutWindow = window.open(logoutURL + '?access_token=' + token + '&next=' + logoutRedirectURL, '_blank', 'location=no');
            if (runningInCordova) {
                setTimeout(function() {
                    logoutWindow.close();
                }, 700);
